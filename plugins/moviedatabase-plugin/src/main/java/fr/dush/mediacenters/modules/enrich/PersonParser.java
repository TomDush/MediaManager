...
public class PersonParser {
    ...
    public List<fr.dush.mediamanager.domain.media.video.Person> getCasting(int limit) {
        ...
        final Collection<fr.dush.mediamanager.domain.media.video.Person> casting =
                transform(filter(persons, selectPerson(ACTOR_JOB)), personConverter);

        // If no limit selected
        if (0 == limit) {
            return newArrayList(casting);
        }
    ...
    public List<fr.dush.mediamanager.domain.media.video.Person> getDirectors() {
        // Select and transform actors
        final Collection<fr.dush.mediamanager.domain.media.video.Person> directors =
                transform(filter(persons, selectPerson(DIRECTOR_JOB)), personConverter);

        // If no limit selected
        return newArrayList(directors);
    }

    private Function<Person, fr.dush.mediamanager.domain.media.video.Person> personConverter =
            new Function<Person, fr.dush.mediamanager.domain.media.video.Person>() {

                @Override
                public fr.dush.mediamanager.domain.media.video.Person apply(Person info) {
                    fr.dush.mediamanager.domain.media.video.Person person =
                            new fr.dush.mediamanager.domain.media.video.Person();
                    ...
                    if (isNotBlank(info.getProfilePath())) {
                        person.setPicture(parent.generateArtUrl(ArtType.ACTOR, info.getProfilePath(), info.getName()));
                    }
