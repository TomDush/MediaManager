...
module.directive 'abnTree', ['$timeout', ($timeout)->
  ...
  templateUrl: '/views/directives/abn_tree_template.html' # <--- another way to do this
  ...
  link: (scope, element, attrs)->
    ...
    on_treeData_change = ->
      ...
      add_branch_to_list = (level, branch, visible)->
        ...
        #
        if branch.treeIcon
          tree_icon = branch.treeIcon(branch)
        else if not branch.children or branch.children.length == 0
