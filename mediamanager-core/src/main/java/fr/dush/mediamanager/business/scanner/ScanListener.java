...
/**
 * Listen {@link RootDirectory}'s events to create and scan directory's contents and find valuable medias. <p/> <p>
 * Warning : this bean is accessible only with events, no interface. </p>
 *
...
public class ScanListener {
     ...
     */
    @Subscribe
    public void scanDirectory(ScanRequestEvent event) {
        try {
            final RootDirectory rootDirectory =
                    rootDirectoryManager.createOrUpdateRootDirectory(event.getRootDirectory());

            ScanStatus status = null;
            if (rootDirectory.getLastRefresh() == null) {
                LOGGER.info("Scan new root directory {} ; lastRefresh = {}",
                            rootDirectory,
                            rootDirectory.getLastRefresh());
                // New directory
            ...
            eventBus.post(new ScanResponseEvent(this, event, status));

        } catch (Exception e) {
            LOGGER.error("Can scan rootdirectory {}.", event.getRootDirectory(), e);
            eventBus.post(new ScanResponseEvent(this, event, new ScanStatus("Scan didn't start.", e)));
        }
    ...
    private ScanStatus refresh(ScanRequestEvent event, RootDirectory rootDirectory,
                               Set<String> paths) throws ScanException {
    ...
    private AbstractScanner<?, ?> getAbstractScanner() {
        return applicationContext.getBean(MoviesScanner.class);
    }
}
