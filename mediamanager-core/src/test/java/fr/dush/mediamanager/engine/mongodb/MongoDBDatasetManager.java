...
public class MongoDBDatasetManager {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(MongoDBDatasetManager.class);
    ...
    public void loadCollection(String collectionName, List<String> locations) throws IOException {
        for (String l : locations) {
            final String content =
                    FileUtils.readFileToString(new File(getClass().getClassLoader().getResource(l).getFile()), "UTF8");
            ...
            } else if (parsedFile instanceof BasicDBObject) {
                db.getCollection(collectionName).insert((BasicDBObject) parsedFile);
            } else {
                LOGGER.warn("Can't load file {}: it's not BasicDBList and neither BasicDBObject. It's: {}.",
                            l,
                            parsedFile);
            }
        }
    ...
    public void initializeDataset(DatabaseScript script) {
        ...
        if (isEmpty(collectionName)) {
            throw new ConfigurationException("[TestCofiguration] No collection name specified for : " + script);
        }
