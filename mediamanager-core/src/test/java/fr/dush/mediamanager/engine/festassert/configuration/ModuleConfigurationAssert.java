...
public class ModuleConfigurationAssert extends AbstractAssert<ModuleConfigurationAssert, ModuleConfiguration> {
     ...
     * With a static import, one's can write directly : <code>assertThat(myModuleConfiguration)</code> and get specific
    ...
    public ModuleConfigurationAssert hasFileds(Field... fileds) {
        ...
        if (fileds == null) {
            throw new AssertionError("Expecting fileds parameter not to be null.");
        }
    ...
    public ModuleConfigurationAssert hasNoFileds() {
        ...
        String errorMessage =
                format("Expected actual <%s> not to have fileds but had :\\n%s", actual, actual.getAllFields());

        // check
        if (!actual.getAllFields().isEmpty()) {
    ...
    public ModuleConfigurationAssert hasFieldsSize(int expected) {
        ...
        // we overrides the default error message with a more explicit one
        String errorMessage = format("Expected fields size to be <%d>, but was <%d> : %s",
                                     expected,
                                     actual.getAllFields().size(),
                                     actual.getAllFields());

        // check
        if (actual.getAllFields().size() != expected) {
            throw new AssertionError(errorMessage);
        }
    ...
    public ModuleConfigurationAssert hasName(String name) {
        ...
        // we overrides the default error message with a more explicit one
        String errorMessage =
                format("Expected ModuleConfiguration's name to be <%s> but was <%s>", name, actual.getName());
