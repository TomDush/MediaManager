...
public class RootDirectoryAssert extends AbstractAssert<RootDirectoryAssert, RootDirectory> {
    ...
    /**
     * An entry point for RootDirectoryAssert to follow Fest standard <code>assertThat()</code> statements.<br> With a
     * static import, one's can write directly : <code>assertThat(myRootDirectory)</code> and get specific assertion
    ...
    public RootDirectoryAssert hasEnricher(String enricher) {
        ...
        String errorMessage =
                format("Expected RootDirectory's enricher to be <%s> but was <%s>", enricher, actual.getEnricher());

        // check
        if (!actual.getEnricher().equals(enricher)) {
    ...
    public RootDirectoryAssert hasLastRefresh(Date lastRefresh) {
        ...
        // we overrides the default error message with a more explicit one
        String errorMessage = format("Expected RootDirectory's lastRefresh to be <%s> but was <%s>",
                                     lastRefresh,
                                     actual.getLastRefresh());

        // check
        if (!actual.getLastRefresh().equals(lastRefresh)) {
            throw new AssertionError(errorMessage);
        }
    ...
    public RootDirectoryAssert hasMediaType(MediaType mediaType) {
        ...
        // we overrides the default error message with a more explicit one
        String errorMessage =
                format("Expected RootDirectory's mediaType to be <%s> but was <%s>", mediaType, actual.getMediaType());
    ...
    public RootDirectoryAssert hasNoPaths() {
        ...
        // we overrides the default error message with a more explicit one
        String errorMessage =
                format("Expected actual <%s> not to have paths but had :\\n%s", actual, actual.getPaths());

        // check
        if (!actual.getPaths().isEmpty()) {
            throw new AssertionError(errorMessage);
        }
